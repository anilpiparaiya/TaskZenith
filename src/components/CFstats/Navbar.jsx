import React from 'react';
import { AiFillGithub, AiFillLinkedin } from 'react-icons/ai';

export const Navbar = () => {
  return (
      <div className='bg-navbar p-3 grid grid-cols-2 w-full'>
          <div className='flex items-center ml-2'>
            <img src="/CF_logo.png" alt="logo" style={{ maxWidth: '50px', maxHeight: '40px' }} />
          </div>

          {/* <div className='border border-black flex justify-center items-center'>
              
          </div > */}
          <div className='flex justify-end items-center'> 
            <a href='https://github.com/anilpiparaiya/TaskZenith' target='_blank' rel='noopener noreferrer'>
                <AiFillGithub className='text-4xl ml-1.5 mr-1.5 ' />
            </a>
            <a href='https://www.linkedin.com/in/anilpiparaiya/' target='_blank' rel='noopener noreferrer'>
                <AiFillLinkedin className='text-4xl ml-1.5 mr-1.5' />
            </a>
          </div>
      
    </div>
  );
};
